...
void SymbolicEquations::subtractMatrix(const DenseMatrix& A, const DenseMatrix& B, DenseMatrix& C) {
...
void SymbolicEquations::getNorm(const DenseMatrix& num, RCP<const Basic>& C) {
    DenseMatrix tmp(num.nrows(), num.ncols());
    num.elementwise_mul_matrix(num, tmp);
    C = sqrt(add(tmp.as_vec_basic()));
}

void SymbolicEquations::convertToUnitVector(const DenseMatrix& num, DenseMatrix& C) {
    DenseMatrix tmp(num.nrows(), num.ncols());
    num.elementwise_mul_matrix(num, tmp);
...
void SymbolicEquations::generateContactPotentialPiecewiseFunctions() {
    ...
    vec_basic nodes_vec_p2p{x1s_x, x1s_y, x1s_z, x1e_x, x1e_y, x1e_z};
    ...
    subtractMatrix(x1e, x1s, e1);
    RCP<const Basic> dist_p2p;
    getNorm(e1, dist_p2p);

    // Point to Point   2h - δ < Δ < 2h + δ
    RCP<const Basic> E_p2p =
        pow(mul(div(one, K1), log(add(one, exp(mul(K1, sub(h2, dist_p2p)))))), 2);
    ...
    E_p2p_gradient_func.init(func_p2p_inputs, E_p2p_gradient.as_vec_basic(), symbolic_cse,
                             opt_level);
    ...
    E_p2p_pen_gradient_func.init(func_p2p_inputs, E_p2p_pen_gradient.as_vec_basic(), symbolic_cse,
                                 opt_level);
    E_p2p_pen_hessian_func.init(func_p2p_inputs, E_p2p_pen_hessian.as_vec_basic(), symbolic_cse,
                                opt_level);

    // POINT TO EDGE
    vec_basic nodes_vec_e2p{x1s_x, x1s_y, x1s_z, x1e_x, x1e_y, x1e_z, x2s_x, x2s_y, x2s_z};
    ...
    subtractMatrix(x1s, x2s, e13);
    DenseMatrix e23(num_rows, num_cols);
    subtractMatrix(x1e, x2s, e23);
    ...
    RCP<const Basic> E_e2p =
        pow(mul(div(one, K1), log(add(one, exp(mul(K1, sub(h2, dist_e2p)))))), 2);
    ...
    E_e2p_gradient_func.init(func_e2p_inputs, E_e2p_gradient.as_vec_basic(), symbolic_cse,
                             opt_level);
    ...
    E_e2p_pen_gradient_func.init(func_e2p_inputs, E_e2p_pen_gradient.as_vec_basic(), symbolic_cse,
                                 opt_level);
    E_e2p_pen_hessian_func.init(func_e2p_inputs, E_e2p_pen_hessian.as_vec_basic(), symbolic_cse,
                                opt_level);

    // EDGE TO EDGE
    vec_basic nodes_vec_e2e{x1s_x, x1s_y, x1s_z, x1e_x, x1e_y, x1e_z,
                            x2s_x, x2s_y, x2s_z, x2e_x, x2e_y, x2e_z};
    ...
    E_e2e_gradient_func.init(func_e2e_inputs, E_e2e_gradient.as_vec_basic(), symbolic_cse,
                             opt_level);
    ...
    E_e2e_pen_gradient_func.init(func_e2e_inputs, E_e2e_pen_gradient.as_vec_basic(), symbolic_cse,
                                 opt_level);
    E_e2e_pen_hessian_func.init(func_e2e_inputs, E_e2e_pen_hessian.as_vec_basic(), symbolic_cse,
                                opt_level);
}

void SymbolicEquations::generateFrictionJacobianPiecewiseFunctions() {
    DenseMatrix nodes{
        {x1s_x, x1s_y, x1s_z, x1e_x, x1e_y, x1e_z, x2s_x, x2s_y, x2s_z, x2e_x, x2e_y, x2e_z}};
    ...
    vec_basic ffr_input{x1s_x,  x1s_y,  x1s_z,  x1e_x,  x1e_y,  x1e_z,  x2s_x,  x2s_y,
                        x2s_z,  x2e_x,  x2e_y,  x2e_z,  x1s_x0, x1s_y0, x1s_z0, x1e_x0,
                        x1e_y0, x1e_z0, x2s_x0, x2s_y0, x2s_z0, x2e_x0, x2e_y0, x2e_z0,
                        f1s_x,  f1s_y,  f1s_z,  f1e_x,  f1e_y,  f1e_z,  f2s_x,  f2s_y,
                        f2s_z,  f2e_x,  f2e_y,  f2e_z,  mu,     dt,     K2};

    vec_basic cforces{f1s_x, f1s_y, f1s_z, f1e_x, f1e_y, f1e_z,
                      f2s_x, f2s_y, f2s_z, f2e_x, f2e_y, f2e_z};
    ...
    convertToUnitVector(f1, contact_norm);

    DenseMatrix v1s(3, 1);
    DenseMatrix v1e(3, 1);
    DenseMatrix v2s(3, 1);
    DenseMatrix v2e(3, 1);
    subtractMatrix(x1s, x1s_0, v1s);
    subtractMatrix(x1e, x1e_0, v1e);
    subtractMatrix(x2s, x2s_0, v2s);
    subtractMatrix(x2e, x2e_0, v2e);
    ...
    subtractMatrix(v_rel, tv_rel, tv_rel);

    RCP<const Basic> tv_rel_n;
    getNorm(tv_rel, tv_rel_n);

    DenseMatrix tv_rel_u(3, 1);
    convertToUnitVector(tv_rel, tv_rel_u);

    RCP<const Basic> gamma =
        sub(div(integer(2), add(one, exp(mul(integer(-1), mul(K2, tv_rel_n))))), one);
    ...
    DenseMatrix ffr_vec1({ffr1s.get(0, 0), ffr1s.get(1, 0), ffr1s.get(2, 0), ffr1e.get(0, 0),
                          ffr1e.get(1, 0), ffr1e.get(2, 0), ffr2s.get(0, 0), ffr2s.get(1, 0),
                          ffr2s.get(2, 0), ffr2e.get(0, 0), ffr2e.get(1, 0), ffr2e.get(2, 0)});
    ...
    friction_partials_dfr_dx_sticking_func.init(ffr_input, friction_partial_dfr_dx1.as_vec_basic(),
                                                symbolic_cse, opt_level);
    friction_partials_dfr_dfc_sticking_func.init(
        ffr_input, friction_partial_dfr_dfc1.as_vec_basic(), symbolic_cse, opt_level);
    ...
    DenseMatrix ffr_vec2({ffr1s.get(0, 0), ffr1s.get(1, 0), ffr1s.get(2, 0), ffr1e.get(0, 0),
                          ffr1e.get(1, 0), ffr1e.get(2, 0), ffr2s.get(0, 0), ffr2s.get(1, 0),
                          ffr2s.get(2, 0), ffr2e.get(0, 0), ffr2e.get(1, 0), ffr2e.get(2, 0)});
    ...
    friction_partials_dfr_dx_sliding_func.init(ffr_input, friction_partial_dfr_dx2.as_vec_basic(),
                                               symbolic_cse, opt_level);
    friction_partials_dfr_dfc_sliding_func.init(ffr_input, friction_partial_dfr_dfc2.as_vec_basic(),
                                                symbolic_cse, opt_level);
}
...
void SymbolicEquations::generateFloorFrictionJacobianFunctions() {
    ...
    floor_friction_partials_dfr_dx_func.init(
        ffr_input, floor_friction_partial_dfr_dx.as_vec_basic(), symbolic_cse, opt_level);
    floor_friction_partials_dfr_dfn_func.init(
        ffr_input, floor_friction_partial_dfr_dfn.as_vec_basic(), symbolic_cse, opt_level);
    ...
    floor_friction_partials_gamma1_dfr_dx_func.init(
        ffr_input, floor_friction_g1_partial_dfr_dx.as_vec_basic(), symbolic_cse, opt_level);
    floor_friction_partials_gamma1_dfr_dfn_func.init(
        ffr_input, floor_friction_g1_partial_dfr_dfn.as_vec_basic(), symbolic_cse, opt_level);
}
