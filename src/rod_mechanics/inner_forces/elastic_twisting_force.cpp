    ...
    for (const auto& limb : soft_robots->limbs) {
        ...
        *deltam = *theta_f - *theta_e;
        ...
        for (int i = 1; i < limb->ne; i++) {
            ...
            gradTwist->row(i).segment(4, 3) =
                -(gradTwist->row(i).segment(0, 3) + gradTwist->row(i).segment(8, 3));
            gradTwist->coeffRef(i, 3) = -1;
            gradTwist->coeffRef(i, 7) = 1;
        }

        for (int i = 1; i < limb->ne; i++) {
            value = GJ / limb->voronoi_len(i) *
                    (deltam->coeff(i) + limb->ref_twist(i) - limb->twist_bar(i));
        ...
        *deltam = *theta_f - *theta_e;
        ...
        for (int i = 0; i < joint->ne; i++) {
            for (int j = i + 1; j < joint->ne; j++) {
                ...
                gradTwist->row(curr_iter).segment(0, 3) =
                    -0.5 / norm_e * (joint->kb).row(curr_iter);
                gradTwist->row(curr_iter).segment(8, 3) = 0.5 / norm_f * (joint->kb).row(curr_iter);
                gradTwist->row(curr_iter).segment(4, 3) =
                    -(gradTwist->row(curr_iter).segment(0, 3) +
        ...
        for (int i = 0; i < joint->ne; i++) {
            ...
            for (int j = i + 1; j < joint->ne; j++) {
                ...
                value = GJ / joint->voronoi_len(curr_iter) *
                        (deltam->coeff(curr_iter) + joint->ref_twist(curr_iter) -
                         joint->twist_bar(curr_iter));
...
void ElasticTwistingForce::computeForceAndJacobian(double dt) {
    ...
    for (const auto& limb : soft_robots->limbs) {
        ...
        for (int i = 1; i < limb->ne; i++) {
            ...
            D2mDe2 = -0.25 / norm2_e *
                     (kbLocal * (te + tilde_t).transpose() + (te + tilde_t) * kbLocal.transpose());
            D2mDf2 = -0.25 / norm2_f *
                     (kbLocal * (tf + tilde_t).transpose() + (tf + tilde_t) * kbLocal.transpose());
            D2mDeDf =
                0.5 / (norm_e * norm_f) * (2.0 / chi * teMatrix - kbLocal * tilde_t.transpose());
            ...
            Jtt = GJ * milen *
                  ((deltam->coeff(i) + limb->ref_twist(i) - limb->twist_bar(i)) * DDtwist +
        ...
        for (int i = 0; i < joint->ne; i++) {
            ...
            for (int j = i + 1; j < joint->ne; j++) {
                ...
                D2mDf2 =
                    -0.25 / norm2_f *
                    (kbLocal * (tf + tilde_t).transpose() + (tf + tilde_t) * kbLocal.transpose());
                D2mDeDf = 0.5 / (norm_e * norm_f) *
                          (2.0 / chi * teMatrix - kbLocal * tilde_t.transpose());
                D2mDfDe = D2mDeDf.transpose();
