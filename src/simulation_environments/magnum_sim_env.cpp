...
MagnumSimEnv::MagnumSimEnv(const std::shared_ptr<World>& m_world, const RenderParams& render_params,
    ...
    // Render a floor if necessary. If not, turn on face culling for improved
    // speed.
    if (w_p->floorExists()) {
        ...
        floor->scale(Vector3{8.0f}).rotateX(90.0_degf).translate(Vector3(0.0f, floor_z, 0.0f));

        misc_drawables.push_back(std::make_unique<FlatDrawable>(*floor, flat_shader, floor_mesh,
                                                                drawables, 0xE0E0E0_rgbf));

        grid_mesh = MeshTools::compile(Primitives::grid3DWireframe({15, 15}));
        grid = std::make_unique<Object3D>(&scene);
        grid->scale(Vector3{8.0f}).rotateX(90.0_degf).translate(Vector3(0.0f, floor_z, 0.0f));

        misc_drawables.push_back(std::make_unique<FlatDrawable>(*grid, flat_shader, grid_mesh,
                                                                drawables, 0x000000_rgbf));
    }
    else {
    ...
    misc_drawables.push_back(
        std::make_unique<CoordinateFrame>(*coordinate_frame, vertex_color_shader, drawables));

    camera_object = std::make_unique<Object3D>(&scene);
    camera_object->translate(Vector3::zAxis(5.0f)).rotateX(-15.0_degf).rotateY(30.0_degf);
    ...
    last_depth =
        ((camera->projectionMatrix() * camera->cameraMatrix()).transformPoint({}).z() + 1.0f) *
        0.5f;
...
Float MagnumSimEnv::depthAt(const Vector2i& windowPosition) {
    ...
    Image2D data =
        GL::defaultFramebuffer.read(Range2Di::fromSize(fbPosition, Vector2i{1}).padded(Vector2i{2}),
                                    {GL::PixelFormat::DepthComponent, GL::PixelType::Float});

    return Math::min<Float>(data.pixels<Float>().asContiguous());
}

Vector3 MagnumSimEnv::unproject(const Vector2i& windowPosition, Float depth) const {
    const Vector2i viewSize = windowSize();
    const Vector2i viewPosition{windowPosition.x(), viewSize.y() - windowPosition.y() - 1};
    const Vector3 in{2 * Vector2{viewPosition} / Vector2{viewSize} - Vector2{1.0f},
                     depth * 2.0f - 1.0f};

    return camera->projectionMatrix().inverted().transformPoint(in);
}

void MagnumSimEnv::keyPressEvent(KeyEvent& event) {
    if (event.key() == KeyEvent::Key::Zero || event.key() == KeyEvent::Key::NumZero) {
        camera_object->resetTransformation()
        ...
        const Vector3 viewTranslation =
            camera_object->transformation().rotationScaling().inverted() *
...
void MagnumSimEnv::mousePressEvent(MouseEvent& event) {
...
void MagnumSimEnv::mouseMoveEvent(MouseMoveEvent& event) {
    if (last_position == Vector2i{-1})
    ...
    if (event.modifiers() & MouseMoveEvent::Modifier::Shift) {
        ...
        camera_object->transformLocal(
            Matrix4::translation(rotation_point) * Matrix4::rotationX(-0.01_radf * delta.y()) *
            Matrix4::rotationY(-0.01_radf * delta.x()) * Matrix4::translation(-rotation_point));
    }

    redraw();
}

void MagnumSimEnv::mouseScrollEvent(MouseScrollEvent& event) {
    const Float current_depth = depthAt(event.position());
    const Float depth = current_depth == 1.0f ? last_depth : current_depth;
...
void MagnumSimEnv::renderEdge(Vec3 top_vertex, Vec3 bot_vertex, float radius,
    ...
    capsule_drawables.push_back(std::make_unique<CapsuleDrawable>(*edges.back(), phong_shader,
                                                                  capsule_mesh, drawables, color));
...
void MagnumSimEnv::stepSimulation() {
    ...
    for (const auto& limb : w_p->soft_robots->limbs) {
        ...
        for (int i = 0; i < limb->ne; i++) {
            ...
            renderEdge(limb->getVertex(i + 1), limb->getVertex(i), radius,
    ...
    for (const auto& joint : w_p->soft_robots->joints) {
        for (int i = 0; i < joint->ne; i++) {
            ...
            renderEdge(w_p->soft_robots->limbs[l]->getVertex(n), joint->x,
    ...
    if (record_frames) {
        // Capture the frame
        Image2D image = GL::defaultFramebuffer.read(GL::defaultFramebuffer.viewport(),
                                                    {PixelFormat::RGBA8Unorm});

        std::ostringstream filename;
        filename << render_record_path << "frame_" << std::setw(5) << std::setfill('0') << curr_iter
                 << ".png";
...
void MagnumSimEnv::runSimulation() {
...
FlatDrawable::FlatDrawable(Object3D& object, Shaders::FlatGL3D& shader, GL::Mesh& mesh,
    ...
    : SceneGraph::Drawable3D{object, &drawables}, shader(shader), mesh(mesh), color(color) {
}
...
CoordinateFrame::CoordinateFrame(Object3D& object, Shaders::VertexColorGL3D& shader,
    ...
    : SceneGraph::Drawable3D{object, &drawables}, shader(shader) {
