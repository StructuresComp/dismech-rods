...
namespace Examples {
    ...
    public:
        explicit VertexColorDrawable(Object3D &object, Shaders::VertexColorGL3D &shader, GL::Mesh &mesh,
                                     SceneGraph::DrawableGroup3D &drawables) : SceneGraph::Drawable3D{object,
                                                                                                      &drawables},
...
//                    .draw(_mesh);
    ...
    MouseInteractionExample::MouseInteractionExample(const Arguments &arguments) : Platform::Application{arguments,
                                                                                                         NoCreate} {
...
//        auto cyl = Primitives::cylinderSolid(100, 30, 5.0);
...
//                .scale(Vector3{8.0f});
    ...
    void MouseInteractionExample::keyPressEvent(KeyEvent &event) {
        ...
        if (event.key() == KeyEvent::Key::Zero || event.key() == KeyEvent::Key::NumZero) {
            ...
            if (event.key() == KeyEvent::Key::Seven || event.key() == KeyEvent::Key::NumSeven) /* Top/bottom */
                transformation = Matrix4::rotationX(-90.0_degf * multiplier);
            else if (event.key() == KeyEvent::Key::One || event.key() == KeyEvent::Key::NumOne) /* Front/back */
                transformation = Matrix4::rotationY(90.0_degf - 90.0_degf * multiplier);
            else if (event.key() == KeyEvent::Key::Three || event.key() == KeyEvent::Key::NumThree) /* Right/left */
